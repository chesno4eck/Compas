# Каталог навыков (SFIA 9)

Используются **только** навыки из SFIA 9. Платформенный контекст задаётся ролью и порогами грейдов.

| id | Название | Категория | Уровни | Файл |
| --- | --- | --- | --- | --- |
| PROG | Программирование / разработка ПО | development | 2–6 | [prog.md](prog.md) |
| SWDN | Проектирование ПО | development | 2–6 | [swdn.md](swdn.md) |
| SINT | Интеграция систем и сборка | development | 2–6 | [sint.md](sint.md) |
| TEST | Функциональное тестирование | quality | 1–6 | [test.md](test.md) |
| NFTS | Нефункциональное тестирование | quality | 1–6 | [nfts.md](nfts.md) |
| HCEV | Проектирование пользовательского опыта | ux | 2–6 | [hcev.md](hcev.md) |
| USEV | Оценка пользовательского опыта | ux | 2–6 | [usev.md](usev.md) |
| ACIN | Доступность и инклюзивность | ux | 2–6 | [acin.md](acin.md) |
| METL | Методы и инструменты | platform | 2–6 | [metl.md](metl.md) |
| QUAS | Обеспечение качества | quality | 2–6 | [quas.md](quas.md) |
| RELM | Управление релизами | delivery | 2–6 | [relm.md](relm.md) |
| DBDS | Проектирование баз данных | data | 2–5 | [dbds.md](dbds.md) |
| DTAN | Моделирование и проектирование данных | data | 2–5 | [dtan.md](dtan.md) |
| PROD | Управление продуктом | product | 2–6 | [prod.md](prod.md) |
| REQM | Определение и управление требованиями | product | 2–6 | [reqm.md](reqm.md) |
| RLMT | Управление отношениями со стейкхолдерами | stakeholder | 4–7 | [rlmt.md](rlmt.md) |
| BUSA | Анализ бизнес-ситуации | business | 2–6 | [busa.md](busa.md) |
| FEAS | Оценка осуществимости | business | 2–6 | [feas.md](feas.md) |
| MEAS | Измерение и метрики | analytics | 2–6 | [meas.md](meas.md) |
| DEMG | Управление поставкой | delivery | 3–6 | [demg.md](demg.md) |
